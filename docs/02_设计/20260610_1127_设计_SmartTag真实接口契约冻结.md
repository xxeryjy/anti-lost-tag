...
| `EMAIL_SEND_FAILED` | 500 | 验证码或通知邮件发送失败 |
...
- 密码必须入库为哈希，不保存明文。
- `API_DATA_SOURCE=database` 时，注册成功后写入 `AuthCode` 验证码记录，验证码只保存哈希。
- 未启用真实邮件供应商时仍使用 `AUTH_MOCK_CODE` 写入验证码表，响应 `mockMode = true`，方便本地联调。
- 启用真实邮件供应商时生成 6 位随机验证码并发送邮件，发送失败返回 `EMAIL_SEND_FAILED`。
...
当前接口沿用页面契约，并增加邮件发送元数据：

| 接口 | 成功响应 |
| --- | --- |
| `POST /api/auth/verify-email/request` | `{ "accepted": true, "mockMode": false, "provider": "resend", "expiresAt": "..." }` |
| `POST /api/auth/verify-email/confirm` | `{ "user": { "id": 1, "email": "...", "emailVerifiedAt": "..." } }` |
| `POST /api/auth/forgot-password` | `{ "accepted": true, "mockMode": false, "provider": "resend", "expiresAt": "..." }` |
| `POST /api/auth/reset-password` | `{ "message": "密码重置成功" }` |

真实数据库模式下，`confirm` 与 `reset-password` 只校验未过期、未消费的 `AuthCode`，不再直接接受固定验证码；固定码只在未启用邮件供应商的本地联调模式下写入验证码表。
...
- 接口增加 IP 级基础防刷，默认 `60 次 / 1 分钟`，触发时返回 `RATE_LIMITED` 和 `Retry-After`。
- 非重复扫描先写入 `notificationStatus = PENDING`，邮件发送成功后更新为 `SENT`，供应商失败更新为 `FAILED`。
...
- 成功后写入 `PrivacyMessage`，初始 `deliveryStatus = PENDING`。
- 留言通知邮件发送成功后更新为 `SENT`，供应商失败更新为 `FAILED`。
- 接口另有 IP 级基础防刷，默认 `10 次 / 5 分钟`，用于拦截明显异常请求。
...
1. 开发环境先落本地 `storage/uploads/images`。
2. 返回可被前端直接展示的同源相对 URL，例如 `/api/uploads/image/:filename`。
3. 服务端必须校验真实图片文件头，不能只信浏览器上报的 MIME。
4. 生产环境再切换到 S3 / Cloudflare R2，并保留同等类型、大小和限流校验。
...
| 邮件服务 | 已接入 `none` / `console` / `resend`，后续可扩展 SMTP/SES |
| 上传存储 | 开发本地目录，生产对象存储 |
| 生产环境变量 | `API_DATA_SOURCE=database`，`SESSION_SECRET` 必须是至少 32 位的非默认密钥 |
...
6. 生产环境不能继续使用 mock 数据源或默认 Session 密钥。
